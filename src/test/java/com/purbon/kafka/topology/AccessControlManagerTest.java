...
public class AccessControlManagerTest {
  ...
  public void newProducerACLsCreation() throws IOException {
    ...
    doReturn(new ArrayList<TopologyAclBinding>())
        .when(aclsBuilder)
        .buildBindingsForProducers(users, topicA.toString(), false);
    accessControlManager.apply(topology, plan);
    verify(aclsBuilder, times(1))
        .buildBindingsForProducers(eq(users), eq(topicA.toString()), eq(false));
  }
  ...
  public void newProducerOptimizedACLsCreation() {
    ...
    doReturn(new ArrayList<TopologyAclBinding>())
        .when(aclsBuilder)
        .buildBindingsForProducers(users, project.namePrefix(), true);
    accessControlManager.apply(topology, plan);
    verify(aclsBuilder, times(1))
        .buildBindingsForProducers(eq(users), eq(project.namePrefix()), eq(true));
  }
